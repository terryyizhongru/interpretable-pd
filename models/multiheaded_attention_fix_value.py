...
class MultiHeadedAttention(torch.nn.Module):
    ...
    def __init__(self, query_dim, key_dim, value_dim, num_heads=8, dropout_rate=0.0, attn_type='cross_embed'):
        ...
        self.linear_out = torch.nn.Linear(query_dim, query_dim)
    ...
    def forward_attention(self, value, scores, mask):
        ...
        x = (
            x.transpose(1, 2).contiguous().view(n_batch, -1, self.h * x.shape[-1])
        )
        if self.attn_type in ['cross_embed']:
            x = x.transpose(-2, -1)  # (batch, time1, d_model)
