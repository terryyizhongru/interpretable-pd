...
from .multiheaded_attention_fix_value import MultiHeadedAttention

class CrossFullModelFixValue(torch.nn.Module):
    """SSL Embedding & Temporal Cross-Attention Model.
    """

    def __init__(self, config):
        super(CrossFullModelFixValue, self).__init__()
        ...
        self.value_dim = informed_input_dim
        ...
        self.classifier = torch.nn.Sequential(
            torch.nn.LayerNorm(self.query_dim * 2),
            torch.nn.SiLU(),
            torch.nn.Linear(
                self.query_dim * 2,
    ...
    def forward(self, batch):
        ...
        embed_mha_output = self.embed_mha(ssl_data, all_embed_informed_data, all_embed_informed_data, mask=None) # -- (batch, time, D), (batch, D, F)
        ...
        time_mha_output = self.time_mha(ssl_data, all_time_informed_data, all_time_informed_data, batch['mask_ssl']) # -- (batch, time, D), (batch, T, F)
